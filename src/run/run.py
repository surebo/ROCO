...
def run(_run, _config, _log):
    ...
    args = SN(**_config)
    args.device = "cuda" if args.use_cuda else "cpu" #修改以测试是否为内存泄漏
    #args.device = "cpu"
    #args.use_cuda = False
...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "role_avail_actions": {"vshape": (env_info["n_actions"],), "group": "agents", "dtype": th.int}, #可获得的角色动作
        "probs": {"vshape": (env_info["n_actions"],), "group": "agents", "dtype": th.float},
        "reward": {"vshape": (1,)},
        "terminated": {"vshape": (1,), "dtype": th.uint8},
        "roles": {"vshape": (1,), "group": "agents", "dtype": th.long}, #角色
        ...
        "q_error" : {"vshape":(env_info["n_agents"],),"dtype":th.uint8},
    }
    groups = {
