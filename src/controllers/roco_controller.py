...
class ROCOMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        agent_outputs, role_outputs, _ = self.forward(ep_batch, t_ep, test_mode=test_mode, t_env=t_env)
    ...
    def forward(self, ep_batch, t, test_mode=False, t_env=None, **kwargs):
        agent_inputs = self._build_inputs(ep_batch, t)
        _, self.hidden_states, losses = self.agent.forward(agent_inputs, self.hidden_states, ep_batch.batch_size, 
            test_mode=test_mode, **kwargs)

        # select roles
        _,self.role_hidden_states,_ = self.role_agent.forward(agent_inputs, self.hidden_states, ep_batch.batch_size, 
            test_mode=test_mode, **kwargs)
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), \
            (None if role_outputs is None else role_outputs.view(ep_batch.batch_size, self.n_agents, -1)), losses
